...
<head>
    ...
    <title>Daftar Lapak Bazaar</title>
    <style>
...
<body>
    ...
    <div class="head">
        <div class="head-brand">
            ...
            @endif
            <div>
                <h1>Daftar Lapak Bazaar</h1>
                <p class="muted" style="margin:4px 0 0">{{ $event?->name ?? $site?->site_name ?? 'Portal Warga' }} &middot; {{ $submissions->count() }} lapak terdaftar</p>
            </div>
        </div>
        ...
        <tbody>
            @forelse ($submissions as $s)
                <tr>
                    ...
                    <td>{{ $s->jenis_jualan }}</td>
                    <td>{{ $s->familySubmission?->head_of_family_name ?: '-' }}</td>
                </tr>
            @empty
                <tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:24px">Belum ada warga yang daftar bazaar.</td></tr>
            @endforelse
        </tbody>
    </table>
