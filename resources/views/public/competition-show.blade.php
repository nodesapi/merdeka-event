<x-layouts.public :title="$competition->name" :eventName="$event?->name">
    ...
    @if ($competition->isGroup())
        {{-- Winners (tim), per kategori putra/putri --}}
        @if ($winnerTeamsByGender->isNotEmpty())
            <section class="mt-6">
                ...
                @foreach ($winnerTeamsByGender as $catWinnerTeams)
                    <div class="mt-4">
                        @if ($winnerTeamsByGender->count() > 1)
                            <h3 class="mb-2 flex items-center gap-2 text-sm font-bold text-stone-600"><span class="h-4 w-1.5 rounded-full bg-red-600"></span>Kategori {{ $catWinnerTeams->first()->gender_category_label }}</h3>
                        @endif
                        <div class="grid gap-3 sm:grid-cols-3">
                            @foreach ($catWinnerTeams as $team)
        ...
        {{-- Daftar tim, per kategori putra/putri --}}
        <section class="mt-8">
            ...
            @forelse ($teamsByGender as $groupTeams)
                <div class="mt-5">
                    @if ($teamsByGender->count() > 1)
                        <h3 class="mb-2 flex items-center gap-2 text-sm font-bold text-stone-600"><span class="h-4 w-1.5 rounded-full bg-red-600"></span>Kategori {{ $groupTeams->first()->gender_category_label }}</h3>
                    @endif
            ...
            @empty
                <div class="mt-4 merdeka-card p-8 text-center text-sm text-stone-500">Belum ada tim yang terdaftar untuk lomba ini.</div>
            @endforelse
        </section>
    @else
